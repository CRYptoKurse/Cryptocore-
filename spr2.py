...
def test_1_roundtrip_all_modes():
    ...
    for mode in modes:
        ...
        for size in test_sizes:
            ...
            try:
                # 1. Шифрование (без флага --encrypt)
                enc_args = [
                    '--algorithm', 'aes',
                    '--mode', mode,
                    '--key', key,
...
def test_2_with_provided_iv_corrected():
    ...
    for mode in modes:
        ...
        try:
            # 1. Шифрование с нашим инструментом (без флага --encrypt)
            enc_args = [
                '--algorithm', 'aes',
                '--mode', mode,
                '--key', key,
...
def test_3_simple_iv_handling():
    ...
    for mode in modes:
        ...
        try:
            # 1. Шифруем с фиксированным IV (без флага --encrypt)
            enc_args = [
                '--algorithm', 'aes',
                '--mode', mode,
                '--key', key,
            ...
            if retcode != 0:
                ...
                if test_data == decrypted_data:
                    print(f"  ✅ OK - дешифрование с указанным IV работает")
                    mode_passed += 1
                else:
            ...
            cleanup_files(input_path, encrypted_path, decrypted_path)
