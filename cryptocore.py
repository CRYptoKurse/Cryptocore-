# [file name]: cryptocore.py (обновленная версия с поддержкой ETM и derive)
# [file content begin]
import sys
import argparse
import os
...
class CryptoCore:
    ...
    def derive_key_from_password(self, password=None, salt_hex=None, iterations=100000,
                                 length=32, algorithm='pbkdf2', output_file=None,
                                 password_file=None, env_var=None, verbose=False):
        """
        Получение ключа из пароля с использованием указанного алгоритма.
        """
        # Чтение пароля (приоритет: аргумент > файл > переменная окружения)
        ...
        if password:
            ...
            try:
                with open(password_file, 'r') as f:
                    actual_password = f.readline().rstrip('\n')
            except Exception as e:
            ...
            if actual_password is None:
                print(f"Переменная окружения {env_var} не установлена", file=sys.stderr)
                sys.exit(1)
        else:
            print("Ошибка: пароль должен быть предоставлен через --password, --password-file, или --env-var",
                  file=sys.stderr)
            sys.exit(1)

        # Очистка пароля из памяти после использования (если это строка)
        if isinstance(actual_password, str):
            password_bytes = actual_password.encode('utf-8')
        else:
            password_bytes = actual_password
        ...
        if salt_hex:
            try:
                ...
                print(f"Ошибка в формате соли: {e}", file=sys.stderr)
                sys.exit(1)
        else:
            # Генерация случайной соли 16 байт
            salt = generate_random_bytes(16)
            if verbose:
                print(f"[INFO] Сгенерирована случайная соль: {salt.hex()}")

        # Производный ключ
        if algorithm == 'pbkdf2':
            from kdf.pbkdf2 import pbkdf2_hmac_sha256
            derived_key = pbkdf2_hmac_sha256(password_bytes, salt, iterations, length)
        else:
            print(f"Неподдерживаемый алгоритм: {algorithm}", file=sys.stderr)
            sys.exit(1)

        # Очистка пароля из памяти
        if isinstance(actual_password, str):
            # В Python строки неизменяемы, но можно удалить ссылку
            del actual_password
        if 'password_bytes' in locals():
            # Байты можно попытаться перезаписать
        ...
        if output_file:
            try:
                with open(output_file, 'wb') as f:
                    f.write(derived_key)
                if verbose:
                    print(f"[INFO] Производный ключ записан в {output_file}")
            except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(
        description='Утилита для шифрования/расшифрования данных, вычисления хеш-сумм и получения ключей',
        prog='cryptocore'
    )
    ...
    # Парсер для команды derive
    derive_parser = subparsers.add_parser('derive', help='Получение ключа из пароля')
    derive_parser.add_argument('--password',
                               help='Строка пароля')
    derive_parser.add_argument('--salt',
                               help='Соль в виде шестнадцатеричной строки')
    ...
    derive_parser.add_argument('--algorithm', default='pbkdf2', choices=['pbkdf2'],
                               help='Алгоритм получения ключа (по умолчанию: pbkdf2)')
    derive_parser.add_argument('--output',
                               help='Выходной файл для производного ключа (бинарный)')
    derive_parser.add_argument('--password-file',
                               help='Читать пароль из файла')
    derive_parser.add_argument('--env-var',
                               help='Читать пароль из переменной окружения')
    derive_parser.add_argument('-v', '--verbose', action='store_true',
                               help='Подробный вывод')
